...
public class Main extends PApplet {
            ...
            if(currObstacle.lane() == user.lane()){
                if(currObstacle.bottom() > user.top()){
                    if(currObstacle.top() < user.bottom()){
                        if(currObstacle instanceof RollObject){
                            if(!user.getStatus().equals("r")){
                                gameOver = true;
                            }
                        }
                        else if(currObstacle instanceof jumpObject){
    ...
    public MovingObject generateObject(){
        ...
        if(obstacleType == 1){
            return new RollObject(startX+10, lane);
        }
        else if(obstacleType == 2){
            return new Train(startX+10, lane);
        }
        else if(obstacleType == 3){
            return new jumpObject(startX+10, lane);
        }
        else{
